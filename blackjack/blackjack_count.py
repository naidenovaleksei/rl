...
class BlackjackEnv(gym.Env):
    ...
    def _get_obs(self):
        return (sum_hand(self.player), self.dealer[0], usable_ace(self.player), self.get_count())
    ...
    def get_count(self):
        count = scores[self.dealer[0]] + sum(scores[x] for x in self.player)
        if count > 0:
            return 2
        elif count == 0:
            return 1
